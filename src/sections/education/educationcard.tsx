...
const EducationCard: React.FC<Props> = ({ item }) => {
  ...
  return (
    <div className="max-w-sm p-6 bg-base-100 border border-base-200 rounded-lg shadow-sm hover:shadow-lg transition-all duration-300 w-full h-full">
      {/* Title with Link */}
      <div className="mb-4">
        {item.link ? (
          <a href={item.link} target="_blank" rel="noreferrer">
            <h5 className="mb-2 text-xl font-semibold tracking-tight text-base-content hover:text-info transition-colors">
              {item.title}
            </h5>
          </a>
        ) : (
          <h5 className="mb-2 text-xl font-semibold tracking-tight text-base-content">
            {item.title}
          </h5>
        )}
      </div>
